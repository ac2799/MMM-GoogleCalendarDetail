/* Magic Mirror
 * Module: MMM-GoogleCalendarDetail
 *
  ...
  getDom: function () {
    ...
    events.forEach((event, index) => {
      ...
      if (this.config.showLocation) {
        if (event.location) {
          ...
          if (this.config.displaySymbol) {
            ...
            locDescCell.colSpan = "2";
          }
          locDescCell.innerHTML = this.titleTransform(
            event.location,
          ...
          locationRow.appendChild(locDescCell);
      ...
      if (event.description) {
        ...
        if (index >= startFade) {
          currentFadeStep = index - startFade;
          descriptionRow.style.opacity = 1 - (1 / fadeSteps) * currentFadeStep;
        }
      }
    });
